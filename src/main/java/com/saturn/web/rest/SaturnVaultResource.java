...
public class SaturnVaultResource {
	 ...
	 *         saturnPass, or with status 400 (Bad Request) if the saturnPass has
	 *         already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PostMapping("/saturn-vaults")
	@Timed
	public ResponseEntity<SaturnVaultDTO> createSaturnVault(@Valid @RequestBody SaturnVaultDTO saturnPass)
			throws URISyntaxException {
		log.debug("REST request to save SaturnVault : {}", saturnPass);
		if (saturnPass.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("saturnPass", "idexists",
					"A new saturnPass cannot already have an ID")).body(null);
		}
		...
		return ResponseEntity.created(new URI("/api/saturn-vaults/" + result.getId()))
				.headers(HeaderUtil.createEntityCreationAlert("saturnPass", result.getId().toString())).body(result);
	}
	 ...
	 *         saturnPass, or with status 400 (Bad Request) if the saturnPass is not
	 *         valid, or with status 500 (Internal Server Error) if the saturnPass
	 *         couldnt be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/saturn-vaults")
	@Timed
	public ResponseEntity<SaturnVaultDTO> updateSaturnVault(@Valid @RequestBody SaturnVaultDTO saturnPass)
			throws URISyntaxException {
		...
		return ResponseEntity.ok()
				.headers(HeaderUtil.createEntityUpdateAlert("saturnPass", saturnPass.getId().toString())).body(result);
	}
	 ...
	 * @return the ResponseEntity with status 200 (OK) and the list of saturnPasses
	 *         in body
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the saturnPass,
	...
	public ResponseEntity<SaturnVaultDTO> getSaturnVault(@PathVariable Long id) {
		...
		if (saturnPass.getId() == (long) -401) {
			return new ResponseEntity<SaturnVaultDTO>(saturnPass, HttpStatus.UNAUTHORIZED);
		}
		return Optional.ofNullable(saturnPass).map(result -> new ResponseEntity<>(result, HttpStatus.OK))
	 ...
	 * @return An empty ResponseEntity with headers and the status containing
	...
	public ResponseEntity<Void> importVaultCsv(@RequestParam("file") MultipartFile file) throws IOException {
		...
		if (entryCount < 1) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("saturnPass", "noentry",
					"Unable to find any entries in CSV. Please check your columns.")).body(null);
		}

		return ResponseEntity.ok().headers(
				HeaderUtil.createAlert("Imported " + entryCount.toString() + " entries into the vault!", "saturnPass"))
				.build();
	}
	...
	public void exportVaultCsv(HttpServletResponse response) throws IOException {
		...
		for (SaturnVaultDTO e : page.getContent()) {
			try {
				...
				// Saturn vault does not have individual name/url, so we'll use site as name and
				// leave url blank
