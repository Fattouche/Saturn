...
2. Start selenium server `docker run -d --name selenium --shm-size 2g selenium/standalone-firefox:3.12.0-boron`
3. Build the test image `docker build -t saturn_test -f Dockerfile .`
4. Run the saturn tests `docker run --name saturn_test --link selenium --link saturn -v ${PWD}/..:/saturn saturn_test`
